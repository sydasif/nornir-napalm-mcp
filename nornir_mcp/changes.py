"""Config capture and change orchestration for the write path (spec §8, §9).

:func:`capture_running_config` is the shared capture service: it reads a
device's running configuration and returns it as text. The backup tool
uses it for standalone backups; ``nornir_apply_config`` reuses the same
path as its mandatory pre-change capture (spec §8.2) — no duplicated
capture logic later.

:func:`plan_change` is the pure planning step: per host, lines are
gated by :func:`validate_config_lines` and the platform checked for
ruleset/netmiko capability, producing a :class:`ChangePlan` with no
device I/O.
...
saved before the failure are harmless extra state — they are immutable
and cost nothing to retain.

:func:`dry_run_outcomes` turns a plan into §16.1-labeled outcomes
(``dry_run_mode: planned_commands_only``) without touching devices.

NAPALM is preferred for capture. Platforms without a NAPALM driver fall
back to netmiko's ``show running-config``. Failures raise
:class:`McpError` subclasses and are propagated to the caller, which
decides how to surface them (e.g. per-host error outcomes).
"""
...
# Known device error patterns per platform (D10 honest heuristic). These
# are the patterns we *know* indicate a failed config line; absence of a
# match means "no error detected", never "verified good".
...
_CaptureFn = Callable[[str, str, str], str]
