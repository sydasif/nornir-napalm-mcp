...
class TestExpandConfig:
    """Tests for _expand_config — env-var expansion, path resolution, recursion."""

    def test_str_env_expansion(self, tmp_path: Path) -> None:
        """String values have $VAR expanded."""
    ...
    def test_str_user_expansion(self, tmp_path: Path) -> None:
        """String values have ~ expanded."""
        result = runner._expand_config("~/some/path", tmp_path)
        assert result == str(Path.home() / "some" / "path")

    def test_list_recursion(self, tmp_path: Path) -> None:
    ...
    def test_dict_non_path_passes_through(self, tmp_path: Path) -> None:
        """Non-path dict values recurse without path resolution."""
    ...
    def test_dict_unknown_key_no_resolve(self, tmp_path: Path) -> None:
        ...
        result = runner._expand_config({"plugin": "SimpleInventory"}, tmp_path)
        assert result["plugin"] == "SimpleInventory"

    def test_nested_dict_full_config(self, tmp_path: Path) -> None:
...
@pytest.mark.usefixtures("fake_nornir")
class TestGetNornir:
