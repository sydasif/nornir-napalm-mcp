...
- `responses.py` - Response envelope models (`ToolEnvelope`, `HostOutcome`, `StructuredError`) and byte-based truncation (spec §21/§21.1)
...
- `changes.py` - Write-path orchestration: `capture_running_config()` (shared NAPALM/CLI capture, spec §8.2/§9), `plan_change()` (pure per-host policy/capability gating → `ChangePlan`), `capture_pre_change_backups()` (fail-closed §8.2 pre-change backups), `dry_run_outcomes()` (§16.1 `planned_commands_only` labeling), `parse_config_transcript()` (D10 honest transcript heuristic — never reports a detected error as success, §8.3)
- `runner.py` - Nornir initialization, config loading, caching (`get_nornir()`, `reset_nornir()`); exports `NornirLike` protocol
- `tasks.py` - Task helpers: `_filter_devices()`, `run_nornir_task()`, `_results_to_outcomes()`
- `introspection.py` - NAPALM getter discovery per platform (`list_getters()`)
- `server.py` - FastMCP server instance and the 12 `@mcp.tool()` definitions (surface pinned by `test_tool_surface_is_exactly_twelve_frozen_tools`)
...
`test_tool_surface_is_exactly_twelve_frozen_tools` (`FROZEN_TOOL_NAMES`
in `tests/test_server.py`) — any rename or addition fails CI.

**Testing Approach** (`tests/` directory):

- `conftest.py` - Pytest fixtures that stub Nornir for isolated testing
- `test_server.py` - Unit tests for the MCP tools and inventory listing
...
5. `_results_to_outcomes()` normalizes `AggregatedResult` → `dict[str, HostOutcome]`
